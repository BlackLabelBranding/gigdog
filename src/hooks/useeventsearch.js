	...
	useEffect(() => {
		...
		const fetchEvents = async () => {
			try {
				...
				const withDistance = await Promise.all(
					upcoming.map(async (ev) => {
						...
						if (evLat == null || evLng == null) {
							const evCoords = await getCoordsFromCitiesTable(normUpper(ev.state || st), cleanCity(ev.city));
							if (evCoords) {
								evLat = evCoords.lat;
								evLng = evCoords.lng;
							}
				...
				// 5) filter by radius or exact city/state
				const finalEvents = withDistance
					.filter((ev) => {
						if (radiusMiles > 0) {
							if (haveCenter) return ev.distance != null && ev.distance <= radiusMiles;
							return normLower(cleanCity(ev.city)) === normLower(ct) && normUpper(ev.state) === st;
						}
						return normLower(cleanCity(ev.city)) === normLower(ct) && normUpper(ev.state) === st;
					})
					.sort((a, b) => {
